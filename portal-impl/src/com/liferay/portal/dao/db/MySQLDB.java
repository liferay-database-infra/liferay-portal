...
public class MySQLDB extends BaseDB {
	...
	@Override
	public List<QuerySnapshot> getLockedQuerySnapshots(Connection connection)
		throws SQLException {

		List<QuerySnapshot> lockedQuerySnapshots = new ArrayList<>();

		try (PreparedStatement preparedStatement = connection.prepareStatement(
				_LOCKED_QUERY_SNAPSHOTS_SQL)) {
			...
			try (ResultSet resultSet = preparedStatement.executeQuery()) {
				while (resultSet.next()) {
					...
					lockedQuerySnapshots.add(
						new QuerySnapshot(duration, id, query, schema, state));
				}
			}
		}

		return lockedQuerySnapshots;
	}
	...
	private static final String _LOCKED_QUERY_SNAPSHOTS_SQL =
