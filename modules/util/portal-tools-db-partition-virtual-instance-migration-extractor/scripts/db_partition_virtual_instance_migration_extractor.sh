...
if [ -e /proc/$$/fd/255 ]
then
	DB_PARTITION_MIGRATION_EXTRACTOR_PATH=`readlink /proc/$$/fd/255 2>/dev/null`
fi

if [ ! -n "${DB_PARTITION_MIGRATION_EXTRACTOR_PATH}" ]
then
	DB_PARTITION_MIGRATION_EXTRACTOR_PATH="$0"
fi
