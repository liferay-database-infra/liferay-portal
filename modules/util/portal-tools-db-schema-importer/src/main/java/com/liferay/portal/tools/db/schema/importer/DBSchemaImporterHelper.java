...
import com.liferay.portal.tools.db.schema.importer.jdbc.ConcurrentAutoBatchPreparedStatementUtil;
...
public class DBSchemaImporterHelper {
	...
	public void importDB() throws Exception {
		_sqlConcurrentExecutor.runSQLTemplateConcurrently(
		...
		ConcurrentAutoBatchPreparedStatementUtil.start();

		_copyContent();

		ConcurrentAutoBatchPreparedStatementUtil.stop();

		_sqlConcurrentExecutor.runSQLTemplateConcurrently(
			_targetDataSource, _readFile(new File(_path, "indexes.sql")));

		_sqlConcurrentExecutor.stop();
	}
	...
	private void _copyTableContent(
		...
		try (PreparedStatement preparedStatement1 =
			...
			PreparedStatement preparedStatement2 =
				ConcurrentAutoBatchPreparedStatementUtil.concurrentAutoBatch(
			...
			try (ResultSet resultSet = preparedStatement1.executeQuery()) {
				while (resultSet.next()) {
					for (int i = 0; i < sourceColumnsName.size(); i++) {
						...
						int index = i + 1;
						int sourceType = _sourceColumnsType.get(
							sourceTableName + "." + columnName);
						int targetType = _targetColumnsType.get(
							targetTableName + "." + targetColumnsName.get(i));
						...
						if ((sourceType == Types.BIGINT) ||
							...
							if ((value == 0L) && resultSet.wasNull()) {
								preparedStatement2.setNull(index, targetType);
							...
							if ((targetType == Types.BIGINT) ||
								...
								preparedStatement2.setLong(index, value);
							...
							if ((targetType == Types.BINARY) ||
								...
								preparedStatement2.setBytes(index, value);
								...
								PostgreSQLJDBCUtil.setLargeObject(
									preparedStatement2, index, value);
							...
							if (targetType == Types.BLOB) {
								preparedStatement2.setBlob(index, value);

								continue;
							}
							else if (targetType == Types.BIGINT) {
								PostgreSQLJDBCUtil.setLargeObject(
									preparedStatement2, index,
							...
							if ((targetType == Types.BOOLEAN) ||
								...
								preparedStatement2.setBoolean(index, value);
							...
							if (targetType == Types.CLOB) {
								preparedStatement2.setClob(index, value);
							...
							if (targetType == Types.DECIMAL) {
								preparedStatement2.setBigDecimal(index, value);
							...
							if (targetType == Types.DOUBLE) {
								preparedStatement2.setDouble(index, value);
							...
							if (targetType == Types.FLOAT) {
								preparedStatement2.setFloat(index, value);
							...
							if (targetType == Types.INTEGER) {
								preparedStatement2.setInt(index, value);
							...
							if ((targetType == Types.LONGNVARCHAR) ||
								...
								preparedStatement2.setString(index, value);
							...
							if (targetType == Types.TIMESTAMP) {
								preparedStatement2.setTimestamp(index, value);
							...
							if ((targetType == Types.TINYINT) ||
								...
								preparedStatement2.setShort(index, value);
							...
							throw new PortalException(
								"Invalid type: " + sourceType);
						}

						_setColumn(
							index, preparedStatement2, targetType,
							alternativeValue);
					}
	...
	private final SQLConcurrentExecutor _sqlConcurrentExecutor =
		new SQLConcurrentExecutor(
			sql ->
				StringUtil.count(
					StringUtil.toLowerCase(sql), "create or replace rule") > 0);
