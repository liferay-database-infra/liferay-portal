...
public class QuartzJobDetailsDataCleanupPreupgradeProcessTest
	...
	@Test
	public void testUpgradeWithMultipleQuartzJobDetails() throws Exception {
		String jobName1 = RandomTestUtil.randomString();
		String jobName2 = RandomTestUtil.randomString();
		...
		try (LogCapture logCapture = LoggerTestUtil.configureLog4JLogger(
			...
			runSQL(
				StringBundler.concat(
					...
					"PersistedQuartzSchedulerEngineInstance', '", jobName1,
					"', '", RandomTestUtil.randomString(), "', '",
					"com.liferay.portal.scheduler.quartz.internal.job.",
					"MessageSenderJob', 1, 0, 0, 0)"));
			...
			runSQL(
				StringBundler.concat(
					...
					"JOB_GROUP, JOB_CLASS_NAME, IS_DURABLE, IS_NONCONCURRENT, ",
					"IS_UPDATE_DATA, REQUESTS_RECOVERY) values ('",
					"PersistedQuartzSchedulerEngineInstance', '", jobName2,
					"', '", RandomTestUtil.randomString(), "', '",
					"com.liferay.portal.scheduler.quartz.internal.job.",
					"MessageSenderJob', 1, 0, 0, 0)"));
			...
			Assert.assertTrue(
				logMessages.contains(
					"Found 2 Quartz job details with null JOB_DATA that are " +
						"not Liferay-managed jobs:"));

			Assert.assertTrue(logMessages.contains("Job name: " + jobName1));
			Assert.assertTrue(logMessages.contains("Job name: " + jobName2));

			Assert.assertFalse(
				logMessages.contains("Deleted Quartz job detail"));
		}
		finally {
			runSQL(
				"delete from QUARTZ_JOB_DETAILS where JOB_NAME = '" + jobName1 +
					"'");
			runSQL(
				"delete from QUARTZ_JOB_DETAILS where JOB_NAME = '" + jobName2 +
					"'");
		}
	}

	@Test
	public void testUpgradeWithSingleQuartzJobDetail() throws Exception {
